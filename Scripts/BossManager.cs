...
public class bossManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Enemies = EnemyManager.Instance.Enemies; // Access the Enemies list directly
        Enemies.Clear(); // Clear the list in the EnemyManager
        EnemyManager.Instance.PopulateEnemiesList(); // Populate the list again
        BossIsAlive = true;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < Enemies.Count; i++)
        {
            ...
            if (LockOnTarget)
            {
                ...
                if (!stickMan.GetComponent<memberManager>().Ismember)
                {
                    Enemies.RemoveAt(i);
    ...
    public void RemoveEnemiesFromList()
    {
        for (int i = 0; i < Enemies.Count; i++)
        {
            if (Enemies.ElementAt(i).name == gameObject.name)
            {
               Enemies.RemoveAt(i);
                break;
